...
> [!IMPORTANT]
> **使用前提**：为确保插件能准确感知您的输入节奏，请务必在 QQ 设置中开启：**设置 -> 隐私 -> 我的输入状态可见**。如果此项关闭，插件将无法接收到“正在输入”的信号，可能导致合并逻辑不准确。

该插件专为解决QQ用户“打一段发一段”的习惯导致的 LLM 响应碎片化问题。它能智能拦截碎片消息和“正在输入”状态，并在用户停止输入后将其打包成一条完整的消息发送给 LLM。
